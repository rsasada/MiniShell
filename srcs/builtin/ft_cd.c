...
static void update_pwd(t_app *app, char *cwd) {
    ...
    oldpwd = ft_strjoin("OLDPWD=", ((t_env *) (find_env(&app->env_lst, "PWD")->content))->value);
    if (!pwd || !oldpwd)
        exit_with_error("Malloc");
    add_env(&app->env_lst, pwd);
    add_env(&app->env_lst, oldpwd);
}

static void chdir_oldpwd(t_list *env_list) {
    char *oldpwd_path;

    oldpwd_path = ((t_env *) (find_env(&env_list, "OLDPWD")->content))->value;
    if (oldpwd_path == NULL) {
